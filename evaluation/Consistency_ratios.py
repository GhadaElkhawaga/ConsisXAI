...
def compute_reds(params_df, threshold):
  ...
  for sc in sorting_criteria:       
    selected_feats = params_df.iloc[params_df.index[params_df[sc] >= threshold], params_df.columns.get_loc("feature")]
    selected_weights = params_df.iloc[params_df.index[params_df[sc] >= threshold], params_df.columns.get_loc(sc)]
...
#####################################################################################################################################
def compute_reducts_core(file_name,ds):
      params_df = retrieve_artefact(logs_dir, 'features_scores_%s' %(file_name), '.csv', ';')
      ...
      with open(os.path.join(logs_dir, 'reds_%s_python.json'%(file_name)), 'w') as outfile:
        json.dump(reds, outfile)
      with open(os.path.join(logs_dir, 'core_%s_python.json'%(file_name)), 'w') as outfile:
        json.dump(core, outfile)


#compute Reduct/Core ratios
